...
interface PartnerData {
  ...
  bankAccount: string
  rating: number
  attachments: Array<{ fileName: string; filePath: string; fileSize: number; fileType: string }>
  status: string
  remark: string
}
...
export default function EditPartnerPage() {
  ...
  useEffect(() => {
    fetch(`/api/partners/${partnerId}`)
      .then((r) => {
        if (!r.ok) throw new Error('合作方不存在')
        return r.json()
      })
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 p-6 max-w-4xl mx-auto w-full space-y-6">
        <h1 className="text-2xl font-bold">编辑合作方</h1>
        <Card>
